...
abstract class AESBlock[D, U, EO, EI, B <: Data](implicit p: Parameters) extends LazyModule {
  ...
  lazy val module = new LazyModuleImp(this) {
    ...
    */
    ...
    aes.io.encrypt_data_in.bits := in.bits.data.asTypeOf(aes.io.encrypt_data_in.bits.cloneType)
    //in.bits.last //do nothing with this input

    aes.io.encrypt_data_out.ready := out.ready
...
class TLAESBlock (implicit p: Parameters) extends
  AESBlock[TLClientPortParameters, TLManagerPortParameters, TLEdgeOut, TLEdgeIn, TLBundle]
  with TLDspBlock
//No CSRs required here
